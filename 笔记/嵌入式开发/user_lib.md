...
证明参考   https://www.cnblogs.com/MeltingPot/p/19165461
...
// 1ms定时器内执行，输入原始带噪声角度
float raw_angle = encoder_get_angle();
float smooth_angle = OLS_Smooth(&ols, dt, raw_angle);

// 随时获取角速度，不用重复计算
float speed = Get_OLS_Derivative(&ols);
```
...
绝对值 符号函数 限幅函数 死区函数 角度格式化  四舍五入 等不再赘述